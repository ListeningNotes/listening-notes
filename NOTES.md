...
- [ ] **Nothing in the interface reaches `/compare`, `/submit` or `/shuffle`.** The dot row pointed at them, the foot of the wall pointed at them for a day, and both are gone. All three still work if you type the address. Submit is the one that matters most — it is a visitor's door, and a journal asking for albums with no way to send one is asking rhetorically.
