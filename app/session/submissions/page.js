...
export default function SubmissionsInbox() {
  ...
  return (
    <>
      <style>{`
        .sub-row:hover { background: #f0ede8; }
        .sub-row { transition: background 0.1s; }
      `}</style>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', padding: '32px 24px' }}>
          {loading ? (
            ...
            <div style={{ background: '#fff', border: BORDER, borderRadius: 14, overflow: 'hidden' }}>
              ...
              {filtered.map(s => (
                <div key={s.id} className="sub-row" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 60px 140px 120px', padding: '14px 20px', borderBottom: BORDER, alignItems: 'center' }}>
