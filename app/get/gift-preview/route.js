...
// shows "A gift from" them on a card under the mark. This draws the same card
// for the preview a message makes of the link — the face, A GIFT FROM, the
// name, the present — so the link says whose gift it is before anybody opens
// it (Miyel, 2026-09-23: make the texting card look like the header). /get's
// metadata points here only when the giver's journal answered, so the
// picture and the page say the same thing.
//
  ...
  return new ImageResponse(
    (
      <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: BG }}>
        {/* The mark, as it sits over the card on /get. */}
        <svg width="132" height="77" viewBox="76 96 241 140" xmlns="http://www.w3.org/2000/svg">
        ...
        {/* The card: the face over a plain mark, A GIFT FROM and the name, and
            the present at the far end — /get's, three times the size. */}
        <div style={{ display: 'flex', alignItems: 'center', marginTop: 54, padding: '30px 52px 30px 30px', background: PANEL, border: `2px solid ${LINE}`, borderRadius: 44 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 132, height: 132, borderRadius: 32, overflow: 'hidden', background: BG }}>
            {face
              ? <img src={face} alt="" width={132} height={132} style={{ width: 132, height: 132, objectFit: 'cover', borderRadius: 32 }} />
              : <svg width="62" height="62" viewBox="0 0 256 256"><path fill={FAINT} d={USER} /></svg>}
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', marginLeft: 38, maxWidth: 560 }}>
            <div style={{ display: 'flex', fontFamily: 'DM Mono', fontSize: 26, letterSpacing: '0.2em', color: FAINT }}>A GIFT FROM</div>
            <div style={{ display: 'flex', fontFamily: 'Nunito', fontWeight: 700, fontSize: 68, lineHeight: 1.1, color: INK, marginTop: 8 }}>{name}</div>
          </div>
          <svg width="60" height="60" viewBox="0 0 256 256" style={{ marginLeft: 48 }}>
            <path fill={INK} d={GIFT} />
          </svg>
        </div>
        <div style={{ display: 'flex', marginTop: 46, fontFamily: 'DM Mono', fontSize: 24, letterSpacing: '0.14em', color: FAINT }}>GET YOUR COPY</div>
      </div>
    ),
