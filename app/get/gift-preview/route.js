...
// under it, so the picture needs neither — only the mark, small and centred
// at the top, so somebody who has never seen one knows what the gift is
// (Miyel, same evening; over the words first, then here). /get's metadata points here only when the giver's journal
  ...
  return new ImageResponse(
    (
      <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', padding: '0 80px', background: PANEL }}>
        {/* The mark, small and centred over the top of the picture. */}
        <svg width="84" height="49" viewBox="76 96 241 140" xmlns="http://www.w3.org/2000/svg" style={{ position: 'absolute', top: 42, left: 558 }}>
